...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i<y; i++)
    {
        for(int k = 0; k<x; k++)
        {
            fscanf(fp1,"%c",&img[i][k].r);
            fscanf(fp1,"%c",&img[i][k].g);
            fscanf(fp1,"%c",&img[i][k].b);
        }
    }
    FILE* fp2;
    fp2 = fopen(output,"w");
   ...
   // {
	...
	if (size>=1){
        img2 = calloc(1,ceil(x*size)*ceil(y*size)*sizeof(pixel_rgb));
        	...
        	img2 = calloc(1,floor(x*size*y)*sizeof(pixel_rgb));
        }
        ...
        double width_ratio = (double)(x)/(x*size);
        double height_ratio = (double) (y)/(y*size);
        ...
        for(int i = 0; i<ceil(y*size); i++)
        {
            for(int k = 0; k<ceil(x*size); k++)
            {
                ...
                old_x = (double)i * width_ratio;

                old_y = (double)k * height_ratio;
       ...
       if (size>=1)
     {
        for(int i = 0; i<(x*size)*(y*size); i++)
        {
            ...
            fprintf(fp2,"%c",img2[i].r);
            fprintf(fp2,"%c",img2[i].g);
            fprintf(fp2,"%c",img2[i].b);
    ...
    else
   {
        ...
        for(int i = 0; i<floor(x*y*size); i++)
        {
